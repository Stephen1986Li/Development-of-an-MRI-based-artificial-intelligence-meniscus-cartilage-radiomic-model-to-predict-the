...
JMP_SCORE_API void fillModelMetadata(MODELMD *mm) {
    ...
    strcpy_safe(mm->timestamp, "2023-02-13T04:54:30Z");
}
...
JMP_SCORE_API void fillInputMetadata(FIELDMD *fd) {
    ...
    strcpy_safe(fd[52].name, "Prob[Control]");
    fd[52].type = fnum_type;
    fd[52].datalen = 0;        

    strcpy_safe(fd[53].name, "Prob[Case]");
...
#define PROB_CONTROL_                       indata[52].data.fnum
// Original name was: 'Prob[Case]'
#define PROB_CASE_                          indata[53].data.fnum
