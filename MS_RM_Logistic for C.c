...
JMP_SCORE_API void fillModelMetadata(MODELMD *mm) {
	strcpy_safe(mm->creator,   "拟合名义型 Logistic"); 
    ...
    strcpy_safe(mm->timestamp, "2023-02-02T08:06:13Z");
}
...
JMP_SCORE_API void fillOutputMetadata(FIELDMD *fd) {
    strcpy_safe(fd[0].name, "Prob[Control]");
    fd[0].type = fnum_type;
    fd[0].datalen = 0;        

    strcpy_safe(fd[1].name, "Prob[Case]");
    fd[1].type = fnum_type;
    fd[1].datalen = 0;        
...
#define PROB_CONTROL_              outdata[0].data.fnum
// Original name was: 'Prob[Case]'
#define PROB_CASE_                 outdata[1].data.fnum
// Original name was: '最可能的“itype1”_1'
#define _OUTDATA_0                 outdata[2].data.str  // max string length: 7
...
JMP_SCORE_API void score(PARM *indata, PARM *outdata) {
    ...
    Lin_Control_ = 139.063307307109 + -0.0375375058236927 * ORIGINAL_FIRSTORDER_MINIMUM_FC + 30.1305459902407 * ORIGINAL_GLSZM_SMALLAREAEMPHASIS_FC + -5.17171374967425 * ORIGINAL_SHAPE_ELONGATION_LM + -22.628924144954 * ORIGINAL_SHAPE_SPHERICITY_MM + -183.420385655495 * V455_FC + -31.3312684285548 * V620_FC + 13.0992317650441 * V810_MM + -0.0449927285119966 * WAVELETHHH_FIRSTORDER_MINIMUM_FC + -80.6126275103369 * WAVELETHHH_GLCM_CLUSTERSHADE_MM + -175.668834756151 * WAVELETHHH_GLCM_IMC1_LM + 14.1554437543005 * WAVELETHHH_GLRLM_RUNVARIANCE_MM + -0.0000000065902705011 * WAVELETHHH_GLSZM_LARGEAREAHIGHGR_FC + 8.45187008427271 * WAVELETHHL_FIRSTORDER_MEAN_MM + -2.51117441608486 * WAVELETHHL_GLCM_CLUSTERPROMINENC_FC + -70.5459140508903 * WAVELETHHL_GLCM_CORRELATION_MM + 135.331231912644 * WAVELETHHL_GLCM_IMC1_MM + 68.4181502822914 * WAVELETHHL_GLCM_IMC1_TC + -581.704233452415 * WAVELETHHL_GLDM_SMALLDEPENDENCEL_FC + -635.035033535392 * WAVELETHHL_GLDM_SMALLDEPENDENCEL_MM + -0.072635465822395 * WAVELETHHL_GLRLM_LONGRUNHIGHGRAY_MM + 7.06528813212184 * WAVELETHHL_GLSZM_GRAYLEVELVARIAN_MM + 0.0000034111631823944 * WAVELETHHL_GLSZM_LARGEAREALOWGRA_FC + -23.7669673323275 * WAVELETHHL_GLSZM_SMALLAREAEMPHAS_FC + -2.46793938305455 * WAVELETHLH_FIRSTORDER_MEDIAN_LM + 0.0000000525523637457 * WAVELETHLH_GLSZM_LARGEAREAHIGHGR_TC + -1.61723183048599 * WAVELETHLH_GLSZM_ZONEENTROPY_TC + -0.316146251951614 * WAVELETHLL_FIRSTORDER_KURTOSIS_MM + 2.7258700982166 * WAVELETHLL_FIRSTORDER_SKEWNESS_LM + -0.00363526522397216 * WAVELETHLL_GLSZM_GRAYLEVELNONUNI_FC + -0.0000108940674123602 * WAVELETHLL_GLSZM_LARGEAREALOWGRA_LM + -34.6530768983916 * WAVELETHLL_GLSZM_SMALLAREAEMPHAS_FC + -0.00623430072694499 * WAVELETHLL_NGTDM_BUSYNESS_MM + 0.012526519001378 * WAVELETLHH_FIRSTORDER_MAXIMUM_MM + -1.64028698105191 * WAVELETLHH_FIRSTORDER_MEDIAN_LM + -5.46359479661979 * WAVELETLHH_GLSZM_SMALLAREAEMPHAS_MM + -0.00913903040109286 * WAVELETLHL_FIRSTORDER_MINIMUM_TC + 3.83169686327823 * WAVELETLHL_FIRSTORDER_SKEWNESS_FC + -2.44183012541404 * WAVELETLHL_FIRSTORDER_SKEWNESS_TC + -0.271943462270141 * WAVELETLHL_GLCM_CLUSTERSHADE_FC + -2.22250066849681 * WAVELETLHL_GLSZM_ZONEENTROPY_LM + -1.7335082222592 * WAVELETLHL_GLSZM_ZONEENTROPY_MM + -0.00495276405350047 * WAVELETLLH_FIRSTORDER_MINIMUM_FC + -22.8109063799044 * WAVELETLLH_GLCM_CORRELATION_MM + -49.5476168994885 * WAVELETLLH_GLCM_IMC1_MM + 508.787193443226 * WAVELETLLH_GLDM_SMALLDEPENDENCEL_MM + -4.27604784138928 * WAVELETLLH_GLSZM_ZONEENTROPY_FC + 0.00258474806649526 * WAVELETLLL_GLCM_CLUSTERSHADE_LM + -66.3147077572225 * WAVELETLLL_GLCM_IDN_FC + 0.0000155238629072697 * WAVELETLLL_GLDM_LARGEDEPENDENCEH_LM + 0.0027879963430726 * WAVELETLLL_GLSZM_GRAYLEVELNONUNI_FC + 0.128677844112411 * WAVELETLLL_GLSZM_GRAYLEVELVARIAN_MM + -0.237313019002244 * WAVELETLLL_NGTDM_BUSYNESS_TC;
    ...
    strcpy_safe(_OUTDATA_0, _temp_2);
